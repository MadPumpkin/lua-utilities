...
local TileMap = obj.new("TileMap", function(self, width, height)

function TileMap.create(tilesheet, offset, size)
  local tile_map = {
    offset = offset,
    size = size,
    tilesheet = tilesheet,
...
function TileMap:getViewRectangle()
  ...
  return {self.offset.x, self.offset.y, width, height}
end
...
function TileMap:getTileBoundingBox(x, y)
  ...
  return aabb.box(tx, ty, self.tilesheet.tile.size.width, self.tilesheet.tile.size.height)
end
...
function TileMap:drawTile(override)
